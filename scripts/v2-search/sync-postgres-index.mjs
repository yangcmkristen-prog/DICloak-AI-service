...
try {
  ...
  const publishedVersionId = published.rows[0]?.id ?? null;
  const prior = new Map();
  if (publishedVersionId) {
    const old = await client.query(`select chunk_id, content_hash from ${config.schema}.chunks where index_version_id=$1`, [publishedVersionId]);
  ...
  const prepared = chunks.map((chunk) => {
    ...
    return { chunk, embeddingText: buildEmbeddingText(record, chunk), fullText: buildFullText(record, chunk), metadata, reused: old?.content_hash === chunk.contentHash, vector: null };
  });
  for (const batch of batches(prepared.filter((entry) => !entry.reused))) {
