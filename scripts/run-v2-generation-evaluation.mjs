...
const timeoutMs = Math.max(1_000, Number(process.env.EVALUATION_TIMEOUT_MS) || 60_000);
const selectedCaseIds = new Set((process.env.EVALUATION_CASE_IDS || '').split(',').map((value) => value.trim()).filter(Boolean));
const reportName = process.env.EVALUATION_REPORT_NAME || 'v2-generation-latest';
...
const cases = parsed.cases.filter((item) => item.enabled && (!selectedCaseIds.size || selectedCaseIds.has(item.caseId)));
...
async function execute(item) {
  ...
  const response = await fetch(`${baseUrl}/api/v2/chat`, { method: 'POST', signal: AbortSignal.timeout(timeoutMs), headers: { 'content-type': 'application/json' }, body: JSON.stringify({ message: item.question, history: item.history ? [{ role: 'user', content: item.history }] : [], product: item.product, aiEngine: 'v2', aiEngineVersion: '2.0-phase-6' }) });
  ...
  return { caseId: item.caseId, product: item.product, language: item.language, knowledgeType: item.knowledgeType, answer, knowledgeIds: actualKnowledgeIds, evidenceConfidence: meta.evidenceConfidence, responseStrategy: meta.responseStrategy, retrievalMs: meta.retrievalMs, generationMs: meta.generationMs, knowledgeMatch, mustIncludePass, anyExpressionPass, mustNotIncludePass, preserveExactPass, askPass, passed: knowledgeMatch && mustIncludePass && anyExpressionPass && mustNotIncludePass && preserveExactPass && askPass, firstTokenMs: meta.firstTokenMs, totalMs: meta.totalMs ?? Math.round(performance.now() - startedAt), token: meta.usage?.total_tokens ?? 0, modelCalls: meta.modelCalls ?? 0, retry: Boolean(meta.retry), error: null };
}
...
await writeReport(path.join(root, 'reports', 'evaluation'), reportName, report, rows, 'V2 真实主模型回复评测');
if (reportName === 'v2-generation-latest') await fs.writeFile(path.join(root, 'evaluation-source', 'examples', 'v2-generation-summary.json'), `${JSON.stringify({ generatedAt: report.generatedAt, privacy: report.privacy, limitations: report.limitations, summary }, null, 2)}\n`);
