...
export function CustomerOverview() {
  ...
  return <div className="min-h-full bg-slate-50/70 p-4 md:p-8">
...
function CustomerDetail({ customer, onClose, onSummarize }: { customer: Customer; onClose: () => void; onSummarize: () => void }) {
  return <div className="fixed inset-0 z-50 bg-black/25" onMouseDown={onClose}><aside className="ml-auto flex h-full w-full max-w-3xl flex-col bg-background shadow-2xl" onMouseDown={(event) => event.stopPropagation()}><header className="border-b p-6"><div className="mb-4 flex justify-between"><Button variant="ghost" size="sm" onClick={onClose}><X />关闭</Button><Button size="sm" onClick={onSummarize}><RefreshCw />重新 AI 总结</Button></div><div className="flex items-center gap-4"><Avatar className="size-14"><AvatarFallback className="bg-blue-50 text-blue-700">{customer.initials}</AvatarFallback></Avatar><div><h3 className="text-xl font-bold">{customer.name}</h3><p className="mt-1 flex items-center gap-2 text-sm text-muted-foreground"><MessageCircle className="size-4" />{customer.channel} · {customer.contact} · {customer.region}</p></div></div></header>
    <Tabs defaultValue="profile" className="min-h-0 flex-1"><TabsList className="h-12 w-full justify-start rounded-none border-b bg-background px-6"><TabsTrigger value="profile">客户信息</TabsTrigger><TabsTrigger value="issues">历史问题 ({customer.issues.length})</TabsTrigger><TabsTrigger value="features">功能需求 ({customer.features.length})</TabsTrigger><TabsTrigger value="notes">备注</TabsTrigger></TabsList><div className="h-[calc(100vh-221px)] overflow-y-auto bg-muted/20 p-6">
