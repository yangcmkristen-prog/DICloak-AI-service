...
export function CustomerOverview() {
  ...
  const [columnWidths, setColumnWidths] = useState<Record<string, number>>({
    ...
    scenario: 210, status: 110, createdAt: 130, updatedAt: 170, action: 80,
  });
    ...
    <div className="mx-auto max-w-[1500px]">
      ...
      <Card className="overflow-hidden py-0">
        ...
        <Table className="table-fixed" style={{ width: Object.values(columnWidths).reduce((total, width) => total + width, 0) }}>
          ...
          <TableBody>{visibleCustomers.map((customer) => <TableRow key={customer.id} className="h-[74px] cursor-pointer" onClick={() => setSelectedId(customer.id)}>
            ...
            <TableCell className="overflow-hidden text-ellipsis">{customer.scenario}</TableCell><TableCell><Badge variant="outline" className={statusStyle[customer.status]}>{customer.status}</Badge></TableCell>
...
type EditableCustomerKey = "name" | "contact" | "channel" | "teamId" | "region" | "type" | "status" | "scenario" | "users" | "accounts" | "plan" | "monthlyFee" | "createdAt";
