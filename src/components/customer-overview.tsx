...
export function CustomerOverview() {
  ...
  }), [customers, featureQuery, followUpFrom, followUpTo, issueQuery, query, quickFilter, region, status]);
  ...
  useEffect(() => { setPage(1); setJumpPage("1"); }, [featureQuery, followUpFrom, followUpTo, issueQuery, pageSize, query, quickFilter, region, status]);
  ...
  const exportFilteredCustomers = () => {
    const rows = visibleCustomers.map((customer) => ({
      ...
      客户类型: customer.type, 客户状态: customer.status, 当前套餐: customer.plan, 套餐月费: customer.monthlyFee,
  ...
  return <div className="h-full overflow-y-auto bg-slate-50/70 p-4 md:p-8">
    <div className="mx-auto max-w-[1500px]">
      ...
      <div className="mb-4 flex flex-wrap items-center gap-3 rounded-lg border bg-background p-3"><span className="text-sm font-medium">最近跟进时间</span><Input aria-label="最近跟进开始时间" type="date" className="w-auto" value={followUpFrom} onChange={(event) => setFollowUpFrom(event.target.value)} /><span className="text-sm text-muted-foreground">至</span><Input aria-label="最近跟进结束时间" type="date" className="w-auto" value={followUpTo} onChange={(event) => setFollowUpTo(event.target.value)} />{followUpFrom || followUpTo ? <Button variant="ghost" size="sm" onClick={() => { setFollowUpFrom(""); setFollowUpTo(""); }}>清除</Button> : null}</div>
      <Card className="overflow-hidden py-0">
        ...
        <Table className="table-fixed" style={{ width: Object.values(columnWidths).reduce((total, width) => total + width, 0) }}>
          ...
          <TableBody>{pagedCustomers.map((customer) => <TableRow key={customer.id} className="h-[74px] cursor-pointer" onClick={() => setSelectedId(customer.id)}>
            ...
            <TableCell className="text-xs text-muted-foreground">{customer.followUps[0]?.date || "—"}</TableCell><TableCell className="text-xs text-muted-foreground">{customer.dueDate}</TableCell><TableCell className="text-xs text-muted-foreground">{customer.createdAt}</TableCell><TableCell className="text-xs text-muted-foreground">{customer.updatedAt}</TableCell>
...
function CustomerImportDialog({ open, onOpenChange, onImported }: { open: boolean; onOpenChange: (open: boolean) => void; onImported: () => Promise<void> }) {
  ...
  return <Dialog open={open} onOpenChange={changeOpen}><DialogContent className="max-h-[90vh] overflow-x-hidden overflow-y-auto sm:max-w-3xl"><DialogHeader><DialogTitle>批量导入客户</DialogTitle><DialogDescription>系统以团队 ID 匹配客户；已有记录更新，未有记录自动创建。</DialogDescription></DialogHeader>
    <section className="min-w-0 space-y-3 rounded-lg border p-4"><div className="flex flex-wrap items-center justify-between gap-3"><div><h4 className="font-medium">导入模板</h4><p className="text-sm text-muted-foreground">请先下载模板并按示例填写。</p></div><Button variant="outline" onClick={downloadCustomerImportTemplate}><Download />下载模板</Button></div>
      <div className="max-w-full overflow-x-auto rounded-md border"><Table className="min-w-max"><TableHeader><TableRow>{customerImportHeaders.map((header) => <TableHead key={header} className="whitespace-nowrap">{header}{header === "团队ID" ? " *" : ""}</TableHead>)}</TableRow></TableHeader><TableBody><TableRow>{["DIC-示例001", "张三", "zhangsan@example.com", "WhatsApp", "中国", "代理商", "活跃", "高阶版", "49.00", "2026-07-31", "2027-07-31", "跨境电商多店铺运营", "10 人", "100 个", "Multilogin", "多账号安全运营", "性价比高", ""].map((value, index) => <TableCell key={`${index}-${value}`} className="whitespace-nowrap text-xs text-muted-foreground">{value || "—"}</TableCell>)}</TableRow></TableBody></Table></div>
...
const emptyCustomerForm = { contactName: "", teamId: "", contactDetail: "", region: "", customerType: "", useCase: "", userScale: "", accountScale: "", currentPlan: "", monthlyFee: "", dueDate: "", competitorUsage: "", coreNeeds: "", selectionReason: "", churnReason: "", notes: "" };
...
function AddCustomerDialog({ open, customers, onOpenChange, onCreated, onExisting }: { open: boolean; customers: Customer[]; onOpenChange: (open: boolean) => void; onCreated: (id: string) => Promise<void>; onExisting: (id: string) => void }) {
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-2xl"><DialogHeader><DialogTitle>添加客户</DialogTitle><DialogDescription>团队 ID 是客户的唯一归纳标识，也可同时录入历史问题和功能需求。</DialogDescription></DialogHeader><div className="grid gap-4 sm:grid-cols-2">
    {([["联系人 *", "contactName"], ["团队 ID *", "teamId"], ["联系方式", "contactDetail"], ["地区", "region"], ["客户类型", "customerType"], ["使用场景", "useCase"], ["用户规模", "userScale"], ["账号规模", "accountScale"], ["当前套餐", "currentPlan"], ["套餐月费", "monthlyFee"], ["到期时间", "dueDate"], ["竞品使用情况", "competitorUsage"], ["核心需求", "coreNeeds"], ["选择原因", "selectionReason"], ["流失原因", "churnReason"]] as Array<[string, keyof typeof emptyCustomerForm]>).map(([label, key]) => <label key={key} className="space-y-1 text-sm"><span>{label}</span>{key === "currentPlan" ? <Select value={form.currentPlan} onValueChange={(value) => update("currentPlan", value)}><SelectTrigger className="w-full"><SelectValue placeholder="选择套餐" /></SelectTrigger><SelectContent>{planOptions.map((plan) => <SelectItem key={plan} value={plan}>{plan}</SelectItem>)}</SelectContent></Select> : <Input type={key === "dueDate" ? "date" : "text"} value={form[key]} onChange={(event) => update(key, event.target.value)} />}</label>)}
...
type EditableCustomerKey = "name" | "contact" | "channel" | "teamId" | "region" | "type" | "status" | "scenario" | "users" | "accounts" | "plan" | "monthlyFee" | "createdAt" | "dueDate" | "competitorUsage" | "coreNeeds" | "selectionReason" | "churnReason";
