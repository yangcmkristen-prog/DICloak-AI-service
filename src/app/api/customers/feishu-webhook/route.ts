...
import { changedFeishuCustomerFields, feishuAutomaticUpdatedAt, parseFeishuCustomerUpdates } from "@/lib/feishu-customer-webhook";
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    for (const incoming of parsed.updates) {
      ...
      const nonEmptyFields = Object.fromEntries(Object.entries(incoming).filter(([field, value]) => field !== "automaticUpdatedAt" && typeof value === "string" && value.trim()));
      const automaticUpdatedAt = feishuAutomaticUpdatedAt(incoming, receivedAt);
      if (existing) {
        ...
        const summary: SummaryRecord = { ...existing.summary, ...nonEmptyFields, teamId: incoming.teamId };
        summary.automaticUpdatedAt = automaticUpdatedAt;
      ...
      const summary = {
        externalChatId, platform: "feishu", customerStatus: "活跃", createdAt: incoming.createdAt || receivedAt,
        updatedAt: "", ...nonEmptyFields, teamId: incoming.teamId, contactName,
        automaticUpdatedAt,
      };
