...
export function selectActiveBuildingVersion(versions: IndexVersionStatus[], published: IndexVersionStatus | undefined, now = Date.now()): IndexVersionStatus | undefined {
  ...
  void now;
  return versions.find((row) => new Date(row.created_at).getTime() > publishedAt && row.status === "building");
}
...
export async function expireStaleWebsiteBuilds(): Promise<void> {
  ...
  try { await client.query(`update ${config.schema}.index_versions set status='failed',error_summary='PUBLISH_TIMEOUT' where status='building' and version like 'website-%' and created_at < now() - interval '5 minutes'`); }
...
export async function publishWebsiteIndex(knowledge: KnowledgeBase, sourceUpdatedAt: string | null, version: string): Promise<void> {
  ...
  try {
    ...
    const resumable = await client.query(`select id from ${config.schema}.index_versions where status='failed' and version like 'website-%' order by created_at desc limit 1`); const resumeId = resumable.rows[0]?.id ?? null;
