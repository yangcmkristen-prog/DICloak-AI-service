...
export async function previewWebsiteIndex(knowledge: KnowledgeBase, sourceUpdatedAt: string | null): Promise<IndexPreview> {
  ...
  try {
    ...
    return { sourceUpdatedAt, total: built.chunks.length, added, changed, removed: [...oldHashes.keys()].filter((id) => !nextIds.has(id)).length, unchanged: built.chunks.length - added - changed, warnings: built.warnings, publishedVersion: published?.version ?? null, buildingVersion: building?.version ?? null, buildingIndexed: building?.indexed_chunks ?? 0, buildingExpected: building?.expected_chunks ?? 0, failedVersion: failed?.version ?? null, failedError: failed?.error_summary ?? null };
...
export async function publishWebsiteIndex(knowledge: KnowledgeBase, sourceUpdatedAt: string | null, version: string): Promise<void> {
  ...
  try {
    ...
    const old = oldId ? await client.query(`select chunk_id,content_hash from ${config.schema}.chunks where index_version_id=$1`, [oldId]) : { rows: [] }; const hashes = new Map<string, string>(old.rows.map((row) => [row.chunk_id, row.content_hash]));
    const records = new Map(built.records.map((record) => [record.id, record])); const prepared = built.chunks.map((chunk) => { const record = records.get(chunk.knowledgeId); return { chunk, embeddingText: buildEmbeddingText(record, chunk), fullText: buildFullText(record, chunk), metadata: buildSearchMetadata(record, chunk), reused: hashes.get(chunk.chunkId) === chunk.contentHash, vector: null as number[] | null }; });
    for (let offset = 0; offset < prepared.length; offset += 64) {
      const batch = prepared.slice(offset, offset + 64); const changed = batch.filter((item) => !item.reused);
      if (changed.length) { const response = await fetch(`${baseUrl}/embeddings`, { method: "POST", headers: { authorization: `Bearer ${apiKey}`, "content-type": "application/json" }, body: JSON.stringify({ model, input: changed.map((item) => item.embeddingText), dimensions: 1536 }) }); if (!response.ok) throw new Error(`Embedding HTTP ${response.status}`); const payload = await response.json() as { data?: Array<{ embedding?: number[] }> }; if (payload.data?.length !== changed.length) throw new Error("Embedding 返回数量不正确"); changed.forEach((item, index) => { item.vector = payload.data?.[index]?.embedding ?? null; }); }
