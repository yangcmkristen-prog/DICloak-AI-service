...
export async function publishWebsiteIndex(knowledge: KnowledgeBase, sourceUpdatedAt: string | null, version: string): Promise<void> {
  ...
  try {
    ...
    const old = oldId ? await client.query<ExistingChunk>(`select chunk_id,content_hash,embedding_text from ${config.schema}.chunks where index_version_id=$1`, [oldId]) : { rows: [] as ExistingChunk[] }; const oldChunks = new Map(old.rows.map((row) => [row.chunk_id, row]));
