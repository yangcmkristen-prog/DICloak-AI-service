...
export function parseQuery(question: string, requestedProduct: "dicloak" | "paraturbo" = "dicloak"): QueryIntent {
  ...
  const explicitFunctionOperation = /(?:修改|编辑|配置|设置|更换|替换|换成|换掉|换|查看|查询|检测|测试|创建|新建|添加|删除|移除|导入|导出|启用|禁用|重置|打开|关闭).{0,20}(?:环境|浏览器配置|代理|IP|成员|用户|团队|分组|Cookie|扩展|插件|指纹|界面|皮肤|主题|外观)|(?:环境|浏览器配置|代理|IP|成员|用户|团队|分组|Cookie|扩展|插件|指纹|界面|皮肤|主题|外观).{0,16}(?:修改|编辑|配置|设置|更换|替换|换|查看|查询|检测|测试|创建|新建|添加|删除|移除|导入|导出|启用|禁用|重置|打开|关闭)|\b(?:edit|modify|configure|change|replace|view|check|test|create|add|delete|remove|import|export|enable|disable|reset|open|close)\w*\b.{0,30}\b(?:profile|environment|proxy|member|user|team|group|cookie|extension|fingerprint|interface|skin|theme|appearance)\w*\b/i.test(normalized);
