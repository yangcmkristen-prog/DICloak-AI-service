...
export function restoreProtectedResponse(response: string, prepared: PreparedTerminologyPipeline): RestoreResult {
  const errors: TerminologyIssue[] = [];
  let output = response;
  for (const marker of prepared.markers) {
    const count = output.split(marker.marker).length - 1;
    if (count < marker.occurrences) errors.push(issue("error", "MARKER_MISSING_OR_MODIFIED", `内部标记丢失或被修改：${marker.marker}`, { knowledgeId: marker.knowledgeId, termId: marker.termId }));
