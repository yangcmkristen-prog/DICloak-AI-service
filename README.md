...
支持字段：`团队ID`、`联系人`（也兼容 `团队名字`）、`联系方式`（也兼容 `用户联系方式`）、`渠道`（也兼容 `私域渠道`）、`创建时间`、`到期时间`、`当前套餐`（也兼容 `套餐`）、`飞书发送时间`（也兼容 `发送时间`、`更新时间`、`自动更新时间`）。`团队ID` 是必填且唯一的识别字段；一次请求中的重复 ID 仅处理第一条。

已有客户只有在至少一个非空业务字段真正变化时才会写入。`automaticUpdatedAt` 只由飞书 Webhook 更新：优先采用请求中明确传入的飞书发送时间；无法修改飞书配置、请求中没有该字段时，以服务端收到 Webhook 的时间作为近似发送时间。完全相同的重复推送返回 `unchanged`，不会刷新时间。后台批量导入以及客户详情中的新增、编辑、跟进或删除操作均不会更新该字段。每次请求的响应都包含 `requestId`，服务端同时输出前缀为 `[Feishu Customer Webhook Audit]` 的结构化日志。线上可在 Vercel 项目 **Logs / Runtime Logs** 中按该前缀、`requestId`、团队 ID 或 `source` 搜索；本地开发时日志显示在 `pnpm dev` 终端。审计日志不记录联系方式。

Windows Git Bash/Git for Windows 如果没有以 UTF-8 发送中文参数，可使用英文字段名 `teamId`、`contactName`、`contactDetail`、`contactMethod`、`createdAt`、`dueDate`、`currentPlan`、`automaticUpdatedAt`；飞书自动化仍可直接使用上面的中文字段名。
